...
#test nos legal move
def test_move_straight_line():  
    board = create_empty_board()
    board[7][4] = ("RED", ["RED", "dark"]) 
...
#verfie que ca s'arrete quand y a un obstacle
def test_move_blocked_by_piece(): 
    board = create_empty_board()
    board[7][4] = ("RED", ["RED", "dark"]) 
    board[5][4] = ("BLUE", ["BLUE", "light"]) 
...
#verifie que ca va bien en diagonale
def test_direction_player_1():
    board = create_empty_board()
...
def test_find_tower():
    board = create_empty_board()
    board[7][0] = ("YELLOW", ["RED", "dark"]) 
    board[0][7] = ("GREEN", ["RED", "light"]) 

    pos_red_0 = find_tower_position(board, "RED", 0)
    assert pos_red_0 == (7, 0) 

    pos_red_1 = find_tower_position(board, "RED", 1)
    assert pos_red_1 == (0, 7)
...
def test_unmakemove():
    ...
    assert board[6][4][1] is None #verifie que la case a bien ete vide
...
def test_evaluate_mobilite():
    ...
    board_bloque[3][4] = ("BLUE", ["BLUE", "light"]) 
...
def test_evaluate_victoire_proche():
    ...
    board_loin[5][4] = ("BLUE", ["BLUE", "light"])
...
def test_evaluate_mobilite_adversaire():
    # L'adversaire est libre
    board_libre = create_empty_board()
    board_libre[7][4] = ("RED", ["RED", "dark"])   
    board_libre[0][4] = ("RED", ["RED", "light"])  
    score_libre = evaluate(board_libre, 0, "RED")
    ...
    board_bloque[1][4] = ("BLUE", ["BLUE", "dark"]) 
    score_bloque = evaluate(board_bloque, 0, "RED")
    
    assert score_bloque > score_libre

def test_negamax_anticipation(): 
    board = create_empty_board()
    board[1][4] = ("RED", ["RED", "dark"])
